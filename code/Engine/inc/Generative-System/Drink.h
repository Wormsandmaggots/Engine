...
public:
	...
	void addComponent(Component *c) override
	{
		...
		if (strcmp(typeid(*c).name(), typeid(ColliderComponent).name()) == 0)
		{
			...
			addedCollider->setOnCollision([this](ColliderComponent *collider)
										  { onTriggerStay(collider); });
			addedCollider->setOnCollisionStart([this](ColliderComponent *collider)
											   { onTriggerEnter(collider); });
			addedCollider->setOnCollisionExit([this](ColliderComponent *collider)
